...
//                return true;
    ...
    private void onServicesDiscoveredComplete(List<BluetoothGattService> services) {
        ...
        /*if (!serviceRefreshed) {
            ...
            boolean batteryContains = false;
            boolean versionContains = false;
            if (gatt != null) {
                batteryContains = gatt.getService(UuidInfo.BATTERY_SERVICE_UUID) != null;
                versionContains = gatt.getService(UuidInfo.VERSION_SERVICE_UUID) != null;
            }

            if (batteryContains && versionContains) {
                onServicesDiscovered(services);
            } else {
                OtaLogger.w("target service not found : " + (batteryContains ? "" : "battery service ") + (versionContains ? "" : "version service"));
                refreshCache();
            }
        } else {
            onServicesDiscovered(services);
    ...
    private void processCommand() {
        ...
        commandType = commandContext.command.getType();
        ...
        int delay = commandContext.command.getDelay();
        if (delay > 0) {
    ...
    synchronized private void processCommand(CommandContext commandContext) {
        ...
        switch (commandType) {
            case READ:
                ...
                this.readCharacteristic(commandContext, command.getServiceUUID(),
                        command.getCharacteristicUUID());
                break;
            case WRITE:
                this.postCommandTimeoutTask();
                this.writeCharacteristic(commandContext, command.getServiceUUID(),
                        command.getCharacteristicUUID(),
            ...
            case READ_DESCRIPTOR:
                this.postCommandTimeoutTask();
                this.readDescriptor(commandContext, command.getServiceUUID(), command.getCharacteristicUUID(), command.getDescriptorUUID());
                break;
            case WRITE_NO_RESPONSE:
                ...
                break;
            case ENABLE_NOTIFY:
                this.enableNotification(commandContext, command.getServiceUUID(),
                        command.getCharacteristicUUID());
                break;
            case DISABLE_NOTIFY:
                this.disableNotification(commandContext, command.getServiceUUID(),
                        command.getCharacteristicUUID());
                break;
        }
    }
    ...
    public void onCharacteristicChanged(BluetoothGatt gatt,
        ...
        if (commandContext != null) {
            ...
            this.onNotify(characteristic.getValue(),
                    commandContext.command.getServiceUUID(),
                    commandContext.command.getCharacteristicUUID(),
                    commandContext.command.getTag());
        }
    }
